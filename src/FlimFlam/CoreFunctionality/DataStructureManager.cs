...
internal class DataStructureManager {
    ...
    private bool VisitEveryEntryCallbackMethodForKnownEntrys(EventEntry ee, int appLogicalIdx) {
        if (currentIdxFinderMatch == ee.GlobalIndex) {
            indexItMatchedOn = appLogicalIdx;
            itMatchedOn = ee;
            return false;
        }
        return true;
    ...
    private long currentIdxFinderMatch; // Default 0
    private EventEntry itMatchedOn; // Default null
    private int indexItMatchedOn = -1;
    ...
    internal bool VisitEveryEventEntryInOrder(VisitEachEntryCallback everyCallback, VisitEachEventryCompletedCallback finalCallback) {
        ...
        long lastIndexIssued = LastUsedGlobalIndex;
        ...
        if (everyCallback != null) {
            while (currentLowestIdx < lastIndexIssued) {
                ...
                currentIdxFinderMatch = currentLowestIdx;
                ...
                if (indexItMatchedOn != -1) {
                    everyCallback(itMatchedOn, null, indexItMatchedOn);
                    itMatchedOn = null; indexItMatchedOn = -1;
                    currentLowestIdx++;
                    beenHereOnce = false;
                    continue;
                } else {
    ...
    /// </summary>
    /// <returns></returns>
    internal long LastUsedGlobalIndex { get; private set; }

    internal DateTime LastMessageRecievedTime { get; private set; } = DateTime.Now;
    ...
    internal long GetNextGlobalIndex() {
        try {
            LastMessageRecievedTime = DateTime.Now;
            return ++LastUsedGlobalIndex;
    ...
    internal long LasatUsedGlobalIndexWas {
        get { return LastUsedGlobalIndex; }
    }
    ...
    /// </summary>
    private TracedApplicationsArrayList TracedApplications { get; set; } = null;

    internal NonTracedApplicationsArrayList NonTracedApplicationEntries { get; set; } = null;
    ...
    internal bool IsValidTracedApplicationIndex(int virtualIndex) {
        if (virtualIndex < 0) {
            return false;  // Less than 0 means not initialised
        }

        if (virtualIndex > tracedAppVirtualIndexLastUsed) {
            return false;  // should not be possible - higher than ever allocated
        }

        if (TracedAppPhysicalIndexFromVirtualIndex(virtualIndex) < 0) {
            return false;  // Within range but does not represent valid application
        }
    ...
    private int tracedAppVirtualIndexLastUsed;
